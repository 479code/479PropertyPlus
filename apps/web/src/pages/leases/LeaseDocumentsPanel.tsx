...
import { type LeaseDocument, type LeaseDocumentType } from '../../features/lease/types';
import {
  useAddLeaseDocument,
  useLeaseDocuments,
  useRemoveLeaseDocument,
} from '../../features/lease/use-lease-support';

const DOCUMENT_TYPES: LeaseDocumentType[] = [
  'LEASE_AGREEMENT',
  'SIGNED_CONTRACT',
  'ADDENDUM',
  'IDENTITY_DOCUMENT',
...
export function LeaseDocumentsPanel({ leaseId }: { leaseId: string }) {
  const { data: documents, isLoading } = useLeaseDocuments(leaseId);
  const addDocument = useAddLeaseDocument(leaseId);
  const removeDocument = useRemoveLeaseDocument(leaseId);
  ...
  const {
    ...
    defaultValues: { documentType: 'LEASE_AGREEMENT', name: '', url: '', description: '' },
  });
  ...
  async function submit(values: FormValues) {
    try {
      ...
      reset({ documentType: 'LEASE_AGREEMENT', name: '', url: '', description: '' });
      setDialogOpen(false);
    } catch {
            ...
            <form className="space-y-4" onSubmit={handleSubmit(submit)} noValidate>
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => setValue('documentType', v as LeaseDocumentType)}
                >
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="url"
                  placeholder="https://cdn.example.com/lease.pdf"
                  {...register('url')}
                />
                {errors.url ? (
