...
import { type PersonDocument, type PersonDocumentType } from '../../features/crm/types';
import {
  useAddPersonDocument,
  usePersonDocuments,
  useRemovePersonDocument,
...
export function PersonDocumentsPanel({ personId }: { personId: string }) {
  const { data: documents, isLoading } = usePersonDocuments(personId);
  const addDocument = useAddPersonDocument(personId);
  const removeDocument = useRemovePersonDocument(personId);
  ...
  async function submit(values: FormValues) {
    try {
      ...
      reset({ documentType: 'OTHER', name: '', url: '', description: '' });
      setDialogOpen(false);
    } catch {
            ...
            <form className="space-y-4" onSubmit={handleSubmit(submit)} noValidate>
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => setValue('documentType', v as PersonDocumentType)}
                >
